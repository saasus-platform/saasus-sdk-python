"""Communication (Feedback) API snapshot tests.

JavaScriptのcommunication.snapshot.tsを参考にしたスナップショットテスト。
...
from tests.e2e.communication.stories import get_communication_stories
from tests.e2e.communication.state import (
    StateManager,
    cleanup_communication_test_artifacts,
    ensure_communication_test_preconditions,
)
...
class TestCommunicationSnapshot:
    """Communication (Feedback) API snapshot tests."""

    @pytest.fixture(scope="class", autouse=True)
    def setup_snapshot_mode(self, test_config, sdk_client):
        mode = get_snapshot_mode()
        apply_snapshot_mode(mode)

        print("\n🚀 Starting SaaSus Communication API Snapshot Tests")
        print("=" * 80)
        print(describe_mode(mode))
        print("=" * 80)

        if not test_config.dry_run:
            ensure_communication_test_preconditions(sdk_client)
        ...
        try:
            if not test_config.dry_run:
                cleanup_communication_test_artifacts(sdk_client)
    ...
    def test_all_stories_snapshot(self, test_config, test_logger, sdk_client, coverage_tracker):
        ...
        stories = get_communication_stories()
        ...
        try:
            ...
            print("✅ All Communication API E2E snapshot tests completed!")
