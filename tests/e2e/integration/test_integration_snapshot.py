"""Integration (EventBridge) API snapshot tests.

JavaScriptのintegration.snapshot.tsを参考にしたスナップショットテスト。
...
from tests.e2e.integration.stories import get_integration_stories
from tests.e2e.integration.state import (
    StateManager,
    delete_settings_quietly,
    ensure_integration_test_preconditions,
)
...
class TestIntegrationSnapshot:
    ...
    def setup_snapshot_mode(self, test_config, sdk_client):
        ...
        print("\n🚀 Starting SaaSus Integration API Snapshot Tests")
        print("=" * 80)
        print(describe_mode(mode))
        print("=" * 80)

        if not test_config.dry_run:
            ensure_integration_test_preconditions(sdk_client)
        ...
        try:
            if not test_config.dry_run:
                delete_settings_quietly(sdk_client)
    ...
    def test_all_stories_snapshot(self, test_config, test_logger, sdk_client, coverage_tracker):
        ...
        stories = get_integration_stories()
        ...
        try:
            ...
            print("✅ All Integration API E2E snapshot tests completed!")
