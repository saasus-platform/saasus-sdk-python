...
class AccountVerification(BaseModel):
    ...
    """
    verification_method: StrictStr = Field(..., description="code: 検証コード(verification code) link: 検証リンク(verification link) ※ 未提供の機能のため、変更・保存はできません(This function is not yet provided, so it cannot be changed or saved.) ")
    sending_to: StrictStr = Field(..., description="email: Eメール(e-mail) sms: SMS smsOrEmail: SMS不可の場合にEメール(email if SMS is not possible) ")
