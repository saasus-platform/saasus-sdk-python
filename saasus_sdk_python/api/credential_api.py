...
class CredentialApi(object):
    ...
    def create_auth_credentials(self, body : Optional[Credentials] = None, **kwargs) -> AuthorizationTempCode:  # noqa: E501
        ...
        引数のIDトークン・アクセストークン・リフレッシュトークンを一時保存し取得用の一時コードを返却する。 一時コードの有効期間は発行から10秒です。  Temporarily save the parameter for the ID token, access token, and refresh token and return a temporary code for obtaining. Temporary codes are valid for 10 seconds from issuance.   # noqa: E501
    ...
    @validate_arguments
    def get_auth_credentials(self, code : Annotated[Optional[StrictStr], Field(description="一時コード(Temp Code)")] = None, auth_flow : Annotated[Optional[StrictStr], Field(description="認証フロー（Authentication Flow） tempCodeAuth: 一時コードを利用した認証情報の取得 refreshTokenAuth: リフレッシュトークンを利用した認証情報の取得 指定されていない場合は tempCodeAuth になります ")] = None, refresh_token : Annotated[Optional[StrictStr], Field(description="リフレッシュトークン(Refresh Token)")] = None, **kwargs) -> Credentials:  # noqa: E501
        """認証・認可情報の取得(Get Authentication/Authorization Information)  # noqa: E501

        一時コードまたはリフレッシュトークンを利用してIDトークン・アクセストークン・リフレッシュトークンを取得する。  Get ID token, access token, and refresh token using a temporary code or a refresh token.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        ...
        :param code: 一時コード(Temp Code)
        :type code: str
        :param auth_flow: 認証フロー（Authentication Flow） tempCodeAuth: 一時コードを利用した認証情報の取得 refreshTokenAuth: リフレッシュトークンを利用した認証情報の取得 指定されていない場合は tempCodeAuth になります 
        :type auth_flow: str
        :param refresh_token: リフレッシュトークン(Refresh Token)
