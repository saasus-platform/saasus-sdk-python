...
class ApiLog(BaseModel):
    ...
    ApiLog
    """
    trace_id: StrictStr = Field(..., description="トレースID")
    api_log_id: StrictStr = Field(...)
    created_at: StrictInt = Field(..., description="APIログ登録日時のエポック秒")
    created_date: StrictStr = Field(..., description="APIログ登録日")
    ttl: StrictInt = Field(..., description="APIログ削除予定エポック秒")
    request_method: StrictStr = Field(..., description="リクエストメソッド")
    saas_id: StrictStr = Field(...)
    api_key: StrictStr = Field(..., description="APIキー")
    response_status: StrictStr = Field(..., description="レスポンスステータス")
    request_uri: StrictStr = Field(..., description="リクエストURI")
    remote_address: StrictStr = Field(..., description="クライアントIPアドレス")
    referer: StrictStr = Field(..., description="リクエストリファラー")
    request_body: StrictStr = Field(..., description="リクエストボディー")
    response_body: StrictStr = Field(..., description="レスポンスボディー")
