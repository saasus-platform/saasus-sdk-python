...
class UserInfoApi(object):
    ...
    @validate_arguments
    def get_user_info_by_email(self, email : Annotated[StrictStr, Field(..., description="Email")], **kwargs) -> UserInfo:  # noqa: E501
        """Get User Info by Email  # noqa: E501

        Get user information by email address.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.get_user_info_by_email(email, async_req=True)
        >>> result = thread.get()
        ...
        kwargs['_return_http_data_only'] = True
        if '_preload_content' in kwargs:
            raise ValueError("Error! Please call the get_user_info_by_email_with_http_info method with `_preload_content` instead and obtain raw data from ApiResponse.raw_data")
        return self.get_user_info_by_email_with_http_info(email, **kwargs)  # noqa: E501

    @validate_arguments
    def get_user_info_by_email_with_http_info(self, email : Annotated[StrictStr, Field(..., description="Email")], **kwargs) -> ApiResponse:  # noqa: E501
        """Get User Info by Email  # noqa: E501
        ...
        >>> thread = api.get_user_info_by_email_with_http_info(email, async_req=True)
        >>> result = thread.get()
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_user_info_by_email" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
